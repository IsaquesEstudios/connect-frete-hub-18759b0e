...
export const setExternalUserEmail = createServerFn({ method: "POST" })
  .inputValidator((data) =>
    z
      .object({
        userId: z.string().uuid(),
  ...
  .handler(async ({ data }): Promise<{ ok: true; email: string }> => {
    ...
    if (!adminProfileRes.ok) throw new Error("Não foi possível validar o administrador.");
    const adminProfiles = (await adminProfileRes.json()) as Array<{ type?: string }>;
    if (adminProfiles[0]?.type !== "admin") throw new Error("Apenas administradores podem alterar emails.");
    ...
    if (!updateRes.ok) {
      ...
      throw new Error(`Não foi possível alterar o email. ${text}`.trim());
    }
